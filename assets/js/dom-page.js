(function(){
  ...
  // Same scoring model you already use
  function computeScore(items, answers, examples, cantRecall){
    ...
    const penaltyEach = 10;
    const noExamplePenalty = 3;
  ...
  function updateScoreUI(items, state){
    ...
    if(verdictEl){
      verdictEl.textContent =
        s.verdict === 'likely'
          ? window.App.t('verdict_likely', 'Likely dominant') + ` (${s.verdictPct}%)`
          : s.verdict === 'unlikely'
            ? window.App.t('verdict_unlikely', 'Unlikely dominant') + ` (${s.verdictPct}%)`
  ...
  async function main(){
    ...
    // which page?
    const pageKey = getQueryParam('page');
    if(!pageKey){
      document.getElementById('pageTitle').textContent = 'Missing ?page=...';
      document.getElementById('pageIntro').textContent = 'Example: dom.html?page=ne_dom';
      return;
    }

    // load cases + locate json
    const casesPath = (window.App.getLang() === 'vi') ? '../data/cases_vi.json' : '../data/cases_en.json';
    const cases = await window.App.loadJSON(casesPath);

    const rel = cases.pages && cases.pages[pageKey];
    if(!rel){
      document.getElementById('pageTitle').textContent = 'Unknown page key: ' + pageKey;
      document.getElementById('pageIntro').textContent = 'Add it to cases_*.json pages.';
      return;
    }

    const domJsonPath = '../data/' + rel;
    const page = await window.App.loadJSON(domJsonPath);

    const titleEl = document.getElementById('pageTitle');
    const introEl = document.getElementById('pageIntro');
    if(titleEl) titleEl.textContent = page.title || pageKey;
    if(introEl) introEl.textContent = page.intro || '';

    const items = page.items || [];
    const list = document.getElementById('questionList');

    // IMPORTANT: preserve storage continuity with your old pages
    const PAGE_ID = pageKey;
